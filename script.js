...
const TDC = (() => {
  ...
  const THEMES = {
    ...
    kpgu: {
      header : { bg:'#fafafa', color:'#000000', fontWeight:'bold' },
      section: { color:'#000000' },
      data   : {},
      alt    : {},
      hl     : {},
      footer : { bg:'#fafafa', color:'#000000', fontWeight:'bold' },
  ...
  /* Download a saved theme as a .json file (so it can be shared / inspected outside the browser) */
  function exportTheme(idx) {
    const th = savedThemes[idx];
    if (!th) return;
    const blob = new Blob([JSON.stringify(th, null, 2)], { type: 'application/json' });
    const safeName = th.name.replace(/[^a-z0-9_-]+/gi, '_');
    saveAs(blob, 'tdc_theme_' + safeName + '.json');
    showStatus('Theme "' + th.name + '" exported');
  }
    ...
    list.innerHTML = savedThemes.map((th, i) => `
      <div class="tdc-saved-pill">
        ...
        <div class="tdc-saved-pill-btns">
          ...
          <button class="tdc-saved-pill-btn" onclick="TDC.exportTheme(${i})" title="Download as .json file">Export</button>
  ...
  return {
    ...
    promptSaveTheme, applyCustomTheme, renameTheme, updateTheme, deleteTheme, exportTheme,
  };

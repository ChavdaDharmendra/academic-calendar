// ==========================================
// ACADEMIC CALENDAR PROJECT SCRIPT
// Extracted from single HTML file
// ==========================================
...
function gen(){
  ...
  document.getElementById('cal-body').innerHTML=semRows(l1,r1,false,'s1')+semRows(l2,r2,true,'s2');
...
const TDC = (() => {
  ...
  /* ── localStorage key ── */
  const LS_TDC = 'tdc_custom_styles_v2';

  /* ══════════════════════════════════════════════════════════════
     INIT — wire table click + keyboard shortcuts
  ══════════════════════════════════════════════════════════════ */
  function init() {
    /* delegate click on the calendar table */
    document.getElementById('cal-body').addEventListener('click', onTableClick);
  ...
  /* Unique ID for a cell: row index + cell index */
  function getCellUID(cell) {
    const tr = cell.closest('tr');
    const tbody = tr.closest('tbody');
    const rows = [...tbody.querySelectorAll('tr')];
    return rows.indexOf(tr) + '_' + cell.cellIndex;
  }
  ...
  function applyFont() {
    ...
    pushUndo();
    cells.forEach(c => {
      if (family) c.style.fontFamily = family;
      if (size)   c.style.fontSize   = size;
    });
  ...
  function toggleStyle(prop) {
    ...
    cells.forEach(c => {
      switch (prop) {
        case 'bold':
          c.style.fontWeight = isOn ? 'bold' : '';
          break;
        case 'italic':
          c.style.fontStyle = isOn ? 'italic' : '';
          break;
        case 'underline': {
          const cur = c.style.textDecoration || '';
          if (isOn) c.style.textDecoration = cur.includes('underline') ? cur : (cur + ' underline').trim();
          else c.style.textDecoration = cur.replace('underline','').trim();
          break;
        }
      }
    });
  ...
  function applyTextColor() {
    ...
    cells.forEach(c => {
      c.style.color = color;
      /* also push color to child inputs so they display correctly */
      c.querySelectorAll('input').forEach(inp => inp.style.color = color);
    });
  ...
  function applyBorder() {
    ...
    cells.forEach(c => {
      if (side === 'all') {
        c.style.border = bdrVal;
      } else if (side === 'none') {
        c.style.border = 'none';
      } else if (side === 'outer') {
        /* apply to all 4, then inner sides handled by adjacent cells */
        c.style.borderTop    = bdrVal;
        c.style.borderBottom = bdrVal;
        c.style.borderLeft   = bdrVal;
        c.style.borderRight  = bdrVal;
      } else {
  ...
  function applyTheme(name) {
    ...
    table.querySelectorAll('input.desc-input,input.date-input').forEach(inp => {
      ...
      if (td) inp.style.color = td.style.color;
    });
  ...
  function load() {
    ...
    try {
      ...
      cells.forEach(c => {
        if (c.style.color) {
          c.querySelectorAll('input').forEach(inp => inp.style.color = c.style.color);
        }
      });
...
/* ── PATCH: extend window.onload to also load form + attach autosave ── */
